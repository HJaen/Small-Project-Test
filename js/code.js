				...
				Object.keys(jsonObject.Contacts).forEach(key => {
					Object.keys(jsonObject.Contacts[key]).forEach(innerKey => {
						...
						newCell.innerHTML += `<td class="col-actions">` + 
											 ...
											 `<button type="button" id="delete" class="`+contactID+`deletebtn icon deletebtn popup" title="Click to delete contact!" onclick="popup('`+deleteRedirect+`', 'confirmDelete, 500, 400);"></button>` +
											 `</td>`;
					});
				});
			}
